...
Если `photoPosition` отсутствует или `none` — **template-проверка** (OpenCV):
- Измерение средней яркости и edge density характерных зон документа
- Скоринг 4 ориентаций, выбор лучшей
- Конфиг зон задаётся для каждого `docType` в нормализованных координатах

**Требует проверки:** надёжность VLM на конкретных типах документов, стабильность ответа по формату, необходимость fallback.

#### Отклонённые альтернативы

- **Tesseract OSD** — исключён. Не различает 0°/180° (фундаментальное ограничение). Дополнительная тяжёлая зависимость.
- **VLM прямой вопрос про угол** — нестабилен на малых моделях; вопрос про позицию объекта работает значительно надёжнее.
...
Ожидаемая скорость на RTX 5070 16GB: ~200–500ms на запрос (нужно проверить).

#### Что нужно проверить

- Стабильность ответа Qwen 3.6 35B A3B по формату (`top-left` и т.д.) на реальных документах в 4 ориентациях.
- Точность определения позиции фото на каждом типе документа.
- Реальная скорость инференса на сервере.
- Надёжность и необходимость fallback для документов без фото.
