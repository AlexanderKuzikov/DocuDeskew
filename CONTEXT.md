...
### 2026-06-28 — Два потока обработки

Документы разделяются на два потока после VLM-классификации:

#### Поток A — полностраничные документы (основная масса входящих)

- Задача: только ротация 0°/90°/180°/270°.
- Инструмент: `sharp` (уже в проекте).
- Источник ориентации: VLM-классификатор на этапе type detection.
- DocuDeskew (convex hull / minAreaRect) для этого потока **не применяется**.

#### Поток B — малые документы (<10 типов: паспорт РФ, СТС, водительское удостоверение и др.)

- Задача: локализация документа на листе + perspective correction.
- Инструмент: `@techstark/opencv-js`.
- Модуль автономен: на входе — изображение, на выходе — файл с установленными требованиями.

**Условия входа для Потока B:**
- Белый или светло-серый фон (артефакт сканера — допустимо).
- `docType` передаётся от VLM-классификатора.

**Пайплайн рабочей копии (Поток B):**
